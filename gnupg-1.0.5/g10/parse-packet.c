     ...
     )
{
    ...
    */
    ...
    if (pkttype == PKT_PUBLIC_KEY || pkttype == PKT_PUBLIC_SUBKEY)
    {
	pkt->pkt.public_key = m_alloc_clear(sizeof *pkt->pkt.public_key );
	rc = parse_key(inp, pkttype, pktlen, hdr, hdrlen, pkt );
    }
    else if (pkttype == PKT_SECRET_KEY || pkttype == PKT_SECRET_SUBKEY)
    {
	pkt->pkt.secret_key = m_alloc_clear(sizeof *pkt->pkt.secret_key );
	rc = parse_key(inp, pkttype, pktlen, hdr, hdrlen, pkt );
    }
    else if (pkttype == PKT_SYMKEY_ENC)
	rc = parse_symkeyenc( inp, pkttype, pktlen, pkt );
    else if (pkttype == PKT_PUBKEY_ENC)
	rc = parse_pubkeyenc(inp, pkttype, pktlen, pkt );
    ...
    else if (pkttype == PKT_ONEPASS_SIG)
    {
	pkt->pkt.onepass_sig = m_alloc_clear(sizeof *pkt->pkt.onepass_sig );
	rc = parse_onepass_sig(inp, pkttype, pktlen, pkt->pkt.onepass_sig );
    }
	...
	rc = parse_user_id(inp, pkttype, pktlen, pkt );
    else if (pkttype == PKT_PHOTO_ID)
    {
	pkt->pkttype = pkttype = PKT_USER_ID;  // must fix it
	rc = parse_photo_id(inp, pkttype, pktlen, pkt);
    }
    else if (pkttype == PKT_OLD_COMMENT || pkttype == PKT_COMMENT)
	rc = parse_comment(inp, pkttype, pktlen, pkt);
    else if (pkttype == PKT_RING_TRUST)
    {
	parse_trust(inp, pkttype, pktlen, pkt);
	rc = 0;
    }
    else if (pkttype == PKT_PLAINTEXT)
	rc = parse_plaintext(inp, pkttype, pktlen, pkt, new_ctb );
    else if (pkttype == PKT_COMPRESSED)
	rc = parse_compressed(inp, pkttype, pktlen, pkt, new_ctb );
    else if (pkttype == PKT_ENCRYPTED || pkttype == PKT_ENCRYPTED_MDC)
	rc = parse_encrypted(inp, pkttype, pktlen, pkt, new_ctb );
    else if (pkttype == PKT_MDC)
	rc = parse_mdc(inp, pkttype, pktlen, pkt, new_ctb );
    else if (pkttype == PKT_GPG_CONTROL)
        rc = parse_gpg_control(inp, pkttype, pktlen, pkt );
    else
	skip_packet(inp, pkttype, pktlen);
...
parse_user_id( IOBUF inp, int pkttype, unsigned long pktlen, PACKET *packet )
{
    ...
    size_t ss = sizeof *packet->pkt.user_id + pktlen;
    if ((ss < sizeof *packet->pkt.user_id) || (ss < pktlen)) {
...
parse_comment( IOBUF inp, int pkttype, unsigned long pktlen, PACKET *packet )
{
    ...
    size_t ss = sizeof *packet->pkt.comment + pktlen - 1;
    if ((ss < sizeof *packet->pkt.comment) || (ss < pktlen - 1)) {
        printf ("possible integer overflow\n");
        abort();
    }
