...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  newTree=(rb_red_blk_tree*) my_malloc(sizeof(rb_red_blk_tree));
  ...
  temp=newTree->nil= (rb_red_blk_node*) my_malloc(sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= (rb_red_blk_node*) my_malloc(sizeof(rb_red_blk_node));
...
void TreeInsertHelp(rb_red_blk_tree* tree, rb_red_blk_node* z) {
  ...
  while( x != nil) {
    ...
    if (1 == tree->Compare(x->key,z->key)) { /* x.key > z.key */
      x=x->left;
    } else if (-1 == tree->Compare(x->key,z->key)) { /* x.key < z.key */
      x=x->right;
    } else {
      break;
    }
  ...
  if ( (y == tree->root) ||
    ...
    y->left=z;
  } else if (-1 == tree->Compare(y->key,z->key)) { /* y.key < z.key */
    y->right=z;
  }
...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  x=(rb_red_blk_node*) my_malloc(sizeof(rb_red_blk_node));
  x->key=key;
  x->info=info;
